...
SWITCHED=0
restore_sources() {
  [[ -d "$APP/src" && ! -L "$APP/src" && -d "$BACKUP/src" ]] || { say 'ROLLBACK FAILED: source paths are invalid'; exit 1; }
  find "$APP/src" -mindepth 1 -maxdepth 1 -exec rm -rf -- {} + || { say 'ROLLBACK FAILED: candidate source could not be removed'; exit 1; }
  cp -a "$BACKUP/src/." "$APP/src/" || { say 'ROLLBACK FAILED: previous source could not be restored'; exit 1; }
...
COPIED=1
find "$APP/src" -mindepth 1 -maxdepth 1 -exec rm -rf -- {} +
cp -a "$BACKUP/candidate-src/." "$APP/src/"
