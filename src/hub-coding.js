...
export async function handleCodingApi({ db, request, response, url, sendJson, readJson, actor, env = process.env, now = () => Date.now() }) {
  ...
  if (!path.startsWith('/api/coding') && !path.startsWith('/api/approvals') && path !== '/api/model-pool') return false;
  try {
    if (request.method === 'GET' && path === '/api/model-pool') {
      return sendJson(response, 200, { ok: true, ...(await modelPoolSnapshot({ db, env, now })) }), true;
    }
