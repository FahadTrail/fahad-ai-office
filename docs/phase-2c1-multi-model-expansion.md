...
| --- | --- | --- | --- | --- | --- | --- |
| Qwen / Model Studio | `qwen3-coder-flash` | OpenAI-compatible Chat Completions; function tools; regional/workspace endpoints; up to 1M context for the coder family | Singapore list price is CNY 2.202 input / CNY 11.009 output per MTok for <=32K. Controller uses rounded USD ceilings of $0.35 / $1.75 to avoid undercounting FX movement. | Dynamic workspace QPM/TPM and account controls | Model Studio states customer data is never used for model training; AES-256 at rest/in platform processing | Eligible after `QWEN_API_PRIVATE_DATA_APPROVED=true` |
